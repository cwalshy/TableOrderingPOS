...
export async function getCheckoutSession(req: Request, res: Response) {
  try {
    ...
    console.log("Unexpected error occurred while purchasing course first: ", error);
    res
      .status(500)
...
export async function createSession(req: Request, res: Response) {
  ...
  try {
    ...
    if (info.product) {
      ...
      sessionConfig = setupPurchaseCourseSession(info, '1989ZhsDuRa5o9jwCniB');
    }
    ...
    console.log("Unexpected error occurred while purchasing course second: ", error);
    res
      .status(500)
      .json({ error: "could not initiate stripe checkout session" });
  }
